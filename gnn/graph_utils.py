...
def safe_float(x):
    """Converts to float, or handles True/False as 1.0/0.0, returns np.nan on error."""
    if str(x).lower() == 'true':
        return 1.0
    ...
    try:
        ...
        return np.nan  # or: raise, or continue, depending on your needs
...
def get_features(id_to_node, node_features):
    ...
    with open(node_features, "r") as fh:
        for line in fh:
            ...
            feats = np.array([safe_float(val) for val in node_feats[1:]])
